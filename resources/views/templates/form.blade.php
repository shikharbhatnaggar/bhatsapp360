...
<form method="POST"
      ...
      x-data="templateBuilder({{ Js::from(['input' => $input, 'template' => ['name' => $template->name, 'language' => $template->language, 'category' => $template->category]]) }})"
    ...
    <div class="space-y-4">
            ...
            <div class="mt-4 grid gap-4 sm:grid-cols-3">
                <div>
                    ...
                    <input id="name" name="name" x-model="name" @input="name = name.toLowerCase().replace(/[^a-z0-9_]/g,'_')"
                           value="{{ old('name', $template->name) }}" @disabled($template->exists) required
                           class="mt-1.5 w-full rounded-lg border-ink-200 text-sm focus:border-jade-600 focus:ring-jade-600 disabled:bg-ink-50">
                    <p class="mt-1 text-xs text-ink-500">{{ $template->exists ? 'Locked once WhatsApp has seen it.' : 'Lowercase, numbers and underscores.' }}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select id="language" name="language" x-model="language"
                            class="mt-1.5 w-full rounded-lg border-ink-200 text-sm focus:border-jade-600 focus:ring-jade-600">
                        @foreach (['en_US' => 'English (US)', 'en_GB' => 'English (UK)', 'hi' => 'Hindi', 'te' => 'Telugu', 'ta' => 'Tamil', 'mr' => 'Marathi'] as $code => $label)
        ...
        <section class="rounded-xl border border-ink-200 bg-white p-5">
            ...
            {{-- Variable mapping: what each placeholder becomes per recipient --}}
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            @endif
            <a href="{{ route('templates.index') }}" class="text-sm text-ink-500 underline underline-offset-2">Cancel</a>
        </div>
    </div>
